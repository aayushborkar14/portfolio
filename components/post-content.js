...
export default function PostContent({ postContent }) {
  const getContentFragment = (index, text, obj, type) => {
    ...
    if (obj) {
      ...
      if (obj.code) {
        modifiedText = (
          <span
            key={index}
            className="font-mono text-pink-600 bg-muted rounded-sm p-1 text-sm"
          >
            {modifiedText}
          </span>
        );
    ...
    switch (type) {
      ...
      case "heading-six":
        return (
          <h6 key={index} className="text-lg font-semibold mb-4">
            {modifiedText.map((item, i) => (
              <React.Fragment key={i}>{item}</React.Fragment>
            ))}
          </h6>
      ...
      case "code-block":
        return (
          <div key={index} className="mb-8">
            <Code code={modifiedText.join("")} />
          </div>
        );
      case "numbered-list":
        return (
          <ol key={index} className="text-lg mb-4 list-decimal list-inside">
            {modifiedText.map((item, i) => (
              <React.Fragment key={i}>{item}</React.Fragment>
            ))}
          </ol>
      ...
      case "bulleted-list":
        return (
          <ul key={index} className="text-lg mb-4 list-disc list-inside">
            {modifiedText.map((item, i) => (
              <React.Fragment key={i}>{item}</React.Fragment>
            ))}
          </ul>
      ...
      case "link":
        return (
          <Link key={index} href={obj.href} className="underline">
            {obj.children.map((item, itemIndex) => (
